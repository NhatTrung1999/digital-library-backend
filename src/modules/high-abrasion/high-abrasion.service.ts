...
export class HighAbrasionService {
    ...
    try {
      ...
      const [rows] = await this.db.query(
        `
          ...
          LEFT JOIN HighAbrasionTestReport f
            ON f.FileID = m.ID
            AND f.IsDeleted = 0
      ...
      const data = (rows as any[]).map((item) => {
        ...
        return {
          ...
          Images: images.map((img: any) => ({
            ...img,
            ImagePath: `${baseUrl}/uploads/HighAbrasion/${img.ImagePath}`,
          })),
        };
      });
      ...
      throw new InternalServerErrorException(
        error?.message || 'Fetch HighAbrasion failed',
      );
    }
  }

  async create(
    dto: CreateHighAbrasionDto,
    files: {
    ...
    try {
      const [HighAbrasion] = await this.db.query(
        `
      ...
      const material = (HighAbrasion as any[])[0];
      ...
      if (values.length) {
        await this.db.query(
          `
              INSERT INTO HighAbrasionImages
  ...
  async removeImage(imageId: string) {
    ...
    try {
      ...
      await this.db.query(
        `
          DELETE FROM HighAbrasionImages
          WHERE ImageID = :imageId
          `,
        {
          replacements: { imageId },
  ...
  async exportExcel(query: any, res: any) {
    try {
      ...
      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet('HighAbrasion');
      ...
      res.setHeader(
        'Content-Disposition',
        'attachment; filename=HighAbrasion.xlsx',
      );
  ...
  async addFile(file: Express.Multer.File, body: any) {
    try {
      ...
      if (existing.length > 0) {
        ...
        await this.db.query(
          `
            INSERT INTO HighAbrasionTestReport
            (
              FileID,
              FileName,
              FilePath,
  ...
  async exportExcelQR(query: any, res: Response) {
    ...
    res.setHeader(
      'Content-Disposition',
      'attachment; filename=HighAbrasion-QR.xlsx',
    );
