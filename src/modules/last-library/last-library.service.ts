...
export class LastLibraryService {
  ...
  async findAll(query: any) {
    try {
      ...
      const pagedIds = (await this.db.query(
      ...
      const ids = pagedIds.map((r) => r.LastLibraryID);
      const idList = ids.map((_, i) => `:id${i}`).join(', ');
      const idReplacements = Object.fromEntries(
        ids.map((id, i) => [`id${i}`, id]),
      );
      ...
      const rows = (await this.db.query(
        `SELECT
           ...
           s.SizeID,    s.Size,
           m.ModelID,   m.Model,
           a.ArticleID, a.Article,
           f.FileName,
           f.FilePath
         FROM LastLibrary ll
         LEFT JOIN LastLibrarySize    s ON s.LastLibraryID = ll.LastLibraryID AND s.IsDeleted = 0
         LEFT JOIN LastLibraryModel   m ON m.SizeID        = s.SizeID         AND m.IsDeleted = 0
         LEFT JOIN LastLibraryArticle a ON a.ModelID        = m.ModelID        AND a.IsDeleted = 0
         LEFT JOIN LastLibrary3DM     f ON f.LastLibraryID = ll.LastLibraryID  AND f.IsDeleted = 0
         WHERE ll.LastLibraryID IN (${idList})
         ORDER BY ll.${sort} ${order}`,
        {
      ...
      for (const row of rows) {
        const key = row.LastLibraryID;
        if (!grouped.has(key)) grouped.set(key, []);
        grouped.get(key)!.push(row);
      }

      const data = ids.map((id) => {
        const recordRows = grouped.get(id) ?? [];
        const item = this.groupLastLibrary(recordRows);

        const firstRow = recordRows[0];
        return {
          ...item,
          FileName: firstRow?.FileName ?? null,
          FilePath: firstRow?.FilePath
            ? `${baseUrl}/uploads/lastlibrary3dm/${firstRow.FilePath}`
            : null,
        };
      });
  ...
  async create(dto: CreateLastLibraryDto, userId: string) {
    ...
    try {
      const lastLibraryId = randomUUID();
      ...
      if (dto.sizes?.length) {
        for (const sizeItem of dto.sizes) {
          const sizeId = randomUUID();

          await this.db.query(
            `INSERT INTO LastLibrarySize (SizeID, LastLibraryID, Size, CreatedBy)
             VALUES (:sizeId, :lastLibraryId, :size, :createdBy)`,
            {
              replacements: {
                sizeId,
                lastLibraryId,
          ...
          if (sizeItem.models?.length) {
            for (const modelItem of sizeItem.models) {
              const modelId = randomUUID();

              await this.db.query(
                `INSERT INTO LastLibraryModel (ModelID, SizeID, Model, CreatedBy)
                 VALUES (:modelId, :sizeId, :model, :createdBy)`,
                {
                  replacements: {
                    modelId,
                    sizeId,
              ...
              if (modelItem.articles?.length) {
                for (const articleItem of modelItem.articles) {
                  const articleId = randomUUID();

                  await this.db.query(
                    `INSERT INTO LastLibraryArticle (ArticleID, ModelID, Article, CreatedBy)
                     VALUES (:articleId, :modelId, :article, :createdBy)`,
                    {
                      replacements: {
                        articleId,
                        modelId,
      ...
      const rows = (await this.db.query(
        `SELECT
         ...
         LEFT JOIN LastLibraryModel   m ON m.SizeID        = s.SizeID         AND m.IsDeleted = 0
         LEFT JOIN LastLibraryArticle a ON a.ModelID        = m.ModelID        AND a.IsDeleted = 0
         WHERE ll.LastLibraryID = :lastLibraryId
           AND ll.IsDeleted = 0`,
        {
      ...
      return this.groupLastLibrary(rows);
    } catch (error) {
  ...
  private groupLastLibrary(rows: any[]) {
    ...
    for (const row of rows) {
      ...
      const sizeEntry = sizesMap.get(row.SizeID);

      if (!row.ModelID) continue;

      if (!sizeEntry.models.has(row.ModelID)) {
        sizeEntry.models.set(row.ModelID, {
          ModelID: row.ModelID,
          Model: row.Model,
          articles: [],
        });
      ...
      if (row.ArticleID) {
        sizeEntry.models.get(row.ModelID).articles.push({
    ...
    try {
      ...
      if (dto.sizes?.length) {
        for (const sizeItem of dto.sizes) {
          if (sizeItem.sizeId) {
            await this.db.query(
              `UPDATE LastLibrarySize
               SET Size = :size, UpdatedAt = GETDATE(), UpdatedBy = :updatedBy
               WHERE SizeID = :sizeId AND IsDeleted = 0`,
              {
                replacements: {
          ...
          if (sizeItem.models?.length) {
            for (const modelItem of sizeItem.models) {
              if (modelItem.modelId) {
                await this.db.query(
                  `UPDATE LastLibraryModel
                   SET Model = :model, UpdatedAt = GETDATE(), UpdatedBy = :updatedBy
              ...
              if (modelItem.articles?.length) {
                for (const articleItem of modelItem.articles) {
                  if (articleItem.articleId) {
                    await this.db.query(
                      `UPDATE LastLibraryArticle
                       SET Article = :article, UpdatedAt = GETDATE(), UpdatedBy = :updatedBy
      ...
      const rows = (await this.db.query(
        `SELECT
         ...
         LEFT JOIN LastLibraryModel   m ON m.SizeID        = s.SizeID         AND m.IsDeleted = 0
         LEFT JOIN LastLibraryArticle a ON a.ModelID        = m.ModelID        AND a.IsDeleted = 0
         WHERE ll.LastLibraryID = :id AND ll.IsDeleted = 0`,
        {
          replacements: { id },
          type: QueryTypes.SELECT,
        },
    ...
    try {
      ...
      await this.db.query(
        `UPDATE LastLibraryArticle
         ...
         WHERE ModelID IN (
           SELECT ModelID FROM LastLibraryModel
           WHERE SizeID IN (
             SELECT SizeID FROM LastLibrarySize
             WHERE LastLibraryID = :id
               AND IsDeleted = 0
           )
           AND IsDeleted = 0
